...
/**
 * Wraps the protected route group. If there's no ID token we redirect
 ...
 *
 * (The ID token is checked here because that's what apiClient now sends as
 * the bearer credential on every API call — see src/lib/apiClient.ts.)
 */
export default function RequireAuth({ children }: { children: ReactNode }) {
  const location = useLocation();

  if (!tokenStore.hasIdToken()) {
