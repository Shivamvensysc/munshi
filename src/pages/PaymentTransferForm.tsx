...
// Reusable Searchable Inline Customer Combobox
interface SearchableSelectProps {
  label: string;
  labelColorClass: string;
  customers: CustomerApiData[];
...
function SearchableCustomerSelect({
  ...
  // Sync display text with selected customer
  useEffect(() => {
    if (selectedCustomer) {
  ...
  const handleSelect = (cust: CustomerApiData) => {
    onSelect(cust.khata_customer_id);
      ...
      <div className="relative">
        <div className="relative flex items-center rounded-xl border border-slate-200 bg-white shadow-2xs transition-all focus-within:border-ledger-brass focus-within:ring-2 focus-within:ring-ledger-brass/15">
          ...
          <input
            ...
            placeholder={
              isLoading ? "Loading customers..." : "Search & Select Customer..."
            }
...
export default function PaymentTransferForm() {
  ...
  // Focus tracking for destination customer input
  const toCustomerInputRef = useRef<HTMLInputElement>(null);
  const [shouldFocusToCustomer, setShouldFocusToCustomer] = useState<boolean>(false);

  useEffect(() => {
    if (shouldFocusToCustomer && !isSubmitting) {
      const timer = setTimeout(() => {
        if (toCustomerInputRef.current) {
          toCustomerInputRef.current.focus();
        }
        setShouldFocusToCustomer(false);
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [shouldFocusToCustomer, isSubmitting]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result.success ?? true) {
        ...
        // Focus directly on the 'To Customer' input field without highlighting text
        setShouldFocusToCustomer(true);
