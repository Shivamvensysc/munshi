...
//         tx.created_at || tx.transaction_date || new Date().toISOString(),
//       ),
//       balance: Math.abs(numBalance).toLocaleString("en-IN"),
...
//   // Color Calculation Logic based on total_lene and total_dene
//   const totalLene = customer ? parseFloat(String((customer as any).total_lene || "0")) : 0;
//   const totalDene = customer ? parseFloat(String((customer as any).total_dene || "0")) : 0;
//   const netBalNum = customer ? parseFloat(String(customer.net_balance || "0")) : 0;
//   const formattedNetBalance = Math.abs(netBalNum).toLocaleString("en-IN");

//   const isOverallLene =
//     (totalLene > 0 && totalDene === 0) ||
//     (totalLene > 0 && totalDene > 0 && totalLene > totalDene) ||
//     customer?.net_status === "YOU_WILL_GET" ||
//     netBalNum > 0;

//   const isOverallDene =
//     (totalDene > 0 && totalLene === 0) ||
//     (totalLene > 0 && totalDene > 0 && totalDene > totalLene) ||
//     customer?.net_status === "YOU_WILL_GIVE" ||
//     netBalNum < 0;

//   const getBalanceColorClass = (balanceVal: number) => {
//     if (balanceVal > 0) return "text-ledger-red";
//     if (balanceVal < 0) return "text-ledger-green";
//     return isOverallLene
//       ? "text-ledger-red"
//       : isOverallDene
//         ? "text-ledger-green"
//         : "text-ledger-muted";
...
//                       {/* Dynamic Balance Color based on Lene (Red) vs Dene (Green) */}
//                       <div
//                         className={`mt-0.5 text-xs tabular-nums ${
//                           hasReferenceNumber ? "font-bold" : "font-semibold"
//                         } ${getBalanceColorClass(item.rawBalance)}`}
//                       >
//                         Balance ₹ {item.balance}
//                       </div>
//                     </div>
...
export default function CustomerDetail() {
  ...
  const mapTransactionToEntry = (
    ...
    // Prioritize updated_at for time and date display
    const rawDisplayDateSource =
      tx.updated_at || tx.created_at || tx.transaction_date || new Date().toISOString();
    ...
    return {
      ...
      customerName: tx.customer_name || "",
      crossEntryPartyName: tx.cross_entry_party_name || "",
      date: formatDateString(rawDisplayDateSource),
      balance: Math.abs(numBalance).toLocaleString("en-IN"),
      rawBalance: numBalance,
      ...(tx.transaction_type === "LENE"
        ? { leneAmount: numAmount.toLocaleString("en-IN") }
