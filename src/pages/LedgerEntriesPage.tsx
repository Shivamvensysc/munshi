...
import { Receipt } from "lucide-react";
import Spinner from "../components/ui/Spinner";
import { customerService, type TransactionApiItem } from "../services";
import type { ApiError } from "../lib/apiClient";
...
interface Entry {
  ...
  description?: string;
  date: string;
  crossEntryParty?: string;
  leneAmount?: string;
  deneAmount?: string;
  status?: string;
}

// Format ISO date to readable string (e.g. 14-09-2026 12:12 PM)
function formatDate(isoString: string | null | undefined): string {
  if (!isoString) return "";
  const dateObj = new Date(isoString);
  if (isNaN(dateObj.getTime())) return isoString;
...
export default function LedgerEntriesList() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      // Direct call via customerService to /api/transactions
      const data = await customerService.getAllTransactions();

      if (data.success && Array.isArray(data.data)) {
        const mappedEntries: Entry[] = data.data.map((item: TransactionApiItem) => {
          ...
          return {
            ...
            date: formatDate(item.created_at || item.transaction_date),
            crossEntryParty: item.cross_entry_party_name?.trim()
              ? item.cross_entry_party_name
              : undefined,
            leneAmount: item.transaction_type === "LENE" ? formattedAmount : undefined,
            deneAmount: item.transaction_type === "DENE" ? formattedAmount : undefined,
            status: item.reference_number ? "CRE" : undefined,
          };
        });
              ...
              entries.map((item, idx) => (
                ...
                >
                  ...
                  <div className="col-span-6 space-y-0.5 sm:col-span-5">
                    ...
                    {/* 4. Cross Entry Party Name */}
                    {item.crossEntryParty && (
                      <div className="text-xs font-semibold text-ledger-brass-dark">
                        Cross Party: {item.crossEntryParty}
                      </div>
                    )}
                  </div>
