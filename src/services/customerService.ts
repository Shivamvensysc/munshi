...
export interface TransactionApiItem {
  ...
  customer_name: string;
  khata_name: string;
  cross_entry_party_name: string | null;
}
...
export const customerService = {
  ...
  /** GET /api/transactions — fetch all transactions across the platform */
  getAllTransactions() {
    return apiClient.get<TransactionApiResponse>("/transactions");
  },

  /** GET /api/transactions?khata_id=:khataId — fetch transactions filtered by Khata */
  getTransactionsByKhata(khataId: string) {
    return apiClient.get<TransactionApiResponse>(`/api/transactions?khata_id=${khataId}`);
  },
};
